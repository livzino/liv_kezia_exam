...
function BackButton({ currentSlide, changeSlide }) {
  ...
  if (currentSlide === 0) {
    return (
      <Link href="/">
        <button className="text-m bg-gradient-to-r from-cyan-400 via-sky-500 to-blue-600  backdrop-blur-md transition-all hover:scale-105 border-4 border-lime-400 rounded-2xl text-lime-400 cursor-pointer h-14 px-4 py-2 w-40 ">BACK</button>
      </Link>
    );
  }
  ...
  if (currentSlide > 0 && currentSlide < 5) {
    return (
      <button className="text-m bg-gradient-to-r from-cyan-400 via-sky-500 to-blue-600  backdrop-blur-md transition-all hover:scale-105 border-4 border-lime-400 rounded-2xl text-lime-400 cursor-pointer h-14 px-4 py-2 w-40" onClick={() => changeSlide("prev")}>
        BACK
      </button>
    );
  }
